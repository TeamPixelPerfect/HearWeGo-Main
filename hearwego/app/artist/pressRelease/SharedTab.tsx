...
const SavedOnesTab = () => {
  ...
  const [sharedPressReleases, setsharedPressReleases] = useState<
    PressReleaseData[]
  >([]);
  ...
  useEffect(() => {
    if (artist?.token) {
      getPressReleasesByArtist(artist.token, artist?.user?.artist_id || "")
        .then((response) => {
          const sharedPressReleases = response.data.filter(
            (pr: any) => pr.Status === "Shared"
          );
          setsharedPressReleases(sharedPressReleases);
  ...
  return (
    <>
      {sharedPressReleases.length > 0 ? (
        <Grid container spacing={2}>
          {sharedPressReleases.map((item, index) => (
