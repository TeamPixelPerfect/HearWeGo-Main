...
const CreateStoreForm = () => {
  ...
  const validationSchema = Yup.object().shape({
    store_banners: Yup.array().of(Yup.mixed().required("Store banner is required")).length(3, "Three store banners are required"),
              ...
              {({ setFieldValue, handleChange }) => (
                <Form>
                  ...
                  <Grid container spacing={3}>
                    {storeData.store_banners.map((storeBanner, index) => (
                      <Grid item xs={12} sm={4} key={index}>
                        <Field name={`store_banners.${index}`}>
                          {({ field }) => (
                            <DropFile
                              fileTypes="image"
                              ...
                              file={storeBanner}
                              setFile={(file) => {
                                const updatedStoreBanners = [...storeData.store_banners];
                                updatedStoreBanners[index] = file;
                                setStoreData((prevData) => ({
                                  ...prevData,
                                  store_banners: updatedStoreBanners,
                                }));
                                setFieldValue(`store_banners.${index}`, file);
                              }}
                              aspectX={16}
                              aspectY={9}
                              shape="rect"
                        ...
                        <ErrorMessage name={`store_banners.${index}`} component="div" style={{ color: 'red' }} />
                      </Grid>
                    ))}
